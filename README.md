...
Sources publish a top-level `skill-manager.json` from Git or from a raw HTTPS archive. A source repository is a catalog that lists those locators; packages appear only after a listed source is added. Manifest v2 describes portable packages of skills and MCP servers without agent-specific destinations; enabled agent profiles determine the projections.
...
- [ADR 0002](docs/decisions/0002-source-repositories-and-locators.md) — catalogs and Git or artifact locators.
