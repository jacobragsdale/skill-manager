...
5. Review every target capability and physical resource. Shared `~/.agents/skills` projections should appear once with several consumers when the planner is using that shared folder. Exclusive skill folders appear per enabled agent otherwise. MCP install is Tier 3; clicking Install is the approval.
